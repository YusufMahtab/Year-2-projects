    ...
    def check_ID(cls, studentID):
        ...
        try:
            if studentIDNumber == 0:
                ...
                print('Student ID found. Continue.\n')
                return True
            else:
                raise ValueError
        except ValueError:
            print('Student ID not found. Please try again or notify a demonstrator.')
            sys.exit(1)
            return False
...
def hashID(ID, mysalt):
    myhash = binascii.hexlify(hashlib.pbkdf2_hmac('sha256',bytes(ID) , bytes(mysalt,'utf-8'), 5000, 16))
...
def checkID(ID, filename):
    ...
    for line in hashes:
        ...
        newhash = hashID(ID, mysalt)
        #print(str(ID)+"new "+newhash+":"+mysalt)
        #print(str(ID)+"old "+myhash+":"+mysalt)
        if newhash == myhash:
            #print("ID found")
            return True
